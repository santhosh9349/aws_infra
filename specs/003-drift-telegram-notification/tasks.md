...
- [x] T001 Create scripts/drift-detection/ directory structure
- [x] T002 Create scripts/drift-detection/requirements.txt with dependencies (python-telegram-bot==20.7, tenacity==8.2.3, pydantic==2.5.0, python-dotenv==1.0.0) - verify exactly 4 core dependencies for minimal dependency principle
- [x] T003 Create scripts/drift-detection/__init__.py
- [x] T004 Create .gitignore entry for scripts/drift-detection/.env
- [x] T005 Create scripts/drift-detection/.env.example with template variables (TELEGRAM_BOT_TOKEN, TELEGRAM_CHANNEL_ID)
...
- [x] T006 Implement DriftDetectionEvent, ResourceChange, TelegramNotification, and NotificationConfig models in scripts/drift-detection/models.py (from data-model.md)
- [x] T007 Implement config.py with NotificationConfig.from_env() method in scripts/drift-detection/config.py
- [x] T008 Implement format_drift_message() and escape_markdown() in scripts/drift-detection/message_formatter.py
- [x] T009 Implement split_message() in scripts/drift-detection/message_formatter.py (split on resource boundaries, add part indicators)
- [x] T010 Implement RetryHandler class in scripts/drift-detection/retry_handler.py (exponential backoff 2s, 4s, 8s)
- [x] T011 Implement TelegramNotifier class in scripts/drift-detection/notify_telegram.py (bot initialization, send logic)
- [x] T012 Add logging with sanitized credentials in scripts/drift-detection/notify_telegram.py (implement sanitize_for_logging() to show only first/last 4 chars, configure structured logging with JSON format: drift_run_id, timestamp, status, action, error_type)
...
- [x] T013 Locate and remove scripts/drift-detection/notify_teams.py (or equivalent MS Teams integration)
- [x] T014 Remove MS Teams dependencies from requirements.txt if present
- [x] T015 Search codebase for MS Teams references and remove (grep -r "teams" "msteams" "microsoft teams")
- [x] T016 Create/update .github/workflows/drift-detection.yml with Telegram notification step
- [x] T017 Add conditional check (if: steps.drift.outputs.drift_detected == 'true') to notification step
- [x] T018 Configure environment variables (TELEGRAM_BOT_TOKEN, TELEGRAM_CHANNEL_ID from secrets)
...
- [x] T019 Create docs/drift-detection/telegram-setup.md with Telegram bot setup guide (BotFather, channel setup, GitHub secrets)
- [x] T020 Update main README.md with Telegram notification section
- [x] T021 Add basic troubleshooting section (common errors: invalid token, bot not in channel, rate limits)
